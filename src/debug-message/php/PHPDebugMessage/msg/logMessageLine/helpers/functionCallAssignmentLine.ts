/**
 * Helper function to calculate the log insertion line for function/method call assignments.
 * Handles multi-line function calls by finding the end of the call expression.
 */
...
export function functionCallAssignmentLine(
  ...
  walk(ast, (node: PHPNode): void => {
    if (isAssign(node)) {
      ...
      // Get the end line of the right-hand side (function call)
      if (right.loc) {
