/**
 * Helper function to calculate the log insertion line for property access assignments.
 * Handles multi-line chained property access by finding the end of the expression.
 */
...
export function propertyAccessAssignmentLine(
  ...
  walk(ast, (node: PHPNode): void => {
    if (isAssign(node)) {
      ...
      // Get the end line of the right-hand side (property access)
      if (right.loc) {
