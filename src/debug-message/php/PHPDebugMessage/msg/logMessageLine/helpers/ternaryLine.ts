/**
 * Helper function to calculate the log insertion line for ternary expression assignments.
 * Handles multi-line ternary expressions by finding the end of the ternary expression.
 */
...
export function ternaryLine(
  ...
  walk(ast, (node: PHPNode): void => {
    if (isAssign(node)) {
      ...
      if (isTernary(right)) {
        // Get the end line of the ternary expression
        if (right.loc) {
