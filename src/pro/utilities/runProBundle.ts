...
export async function runProBundle(
  ...
  patchedBundle = patchedBundle.replace(
    ...
    (match, quote: string) => `${quote}${match.slice(1, -1)}|.py${quote}`,
  );
  ...
  if (typeof turboConsoleLogPro === 'function') {
    try {
      console.log(
        '[Pro] Invoking Pro bundle initialization with Python support...',
      );
      ...
      try {
        ...
        if (pythonFiles.length > 0) {
          console.log(
            '[Pro] Python files detected in workspace, showing notification',
          );
