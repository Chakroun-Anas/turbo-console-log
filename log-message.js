...
function message(
  ...
  const debuggingMsg = `${languageConfig.logMessage}(${quote}${logMessagePrefix}: ${
    insertEnclosingClass ? classThatEncloseTheVar : ""
    }${
    insertEnclosingFunction ? funcThatEncloseTheVar : ""
    }${selectedVar}${quote}${languageConfig.separateText} ${selectedVar}${languageConfig.endVariable})${semicolon}`;
  if (wrapLogMessage) {
    // 16 represents the length of console.log("");
    const wrappingMsg = `${languageConfig.logMessage}(${quote}${logMessagePrefix}: ${"-".repeat(
